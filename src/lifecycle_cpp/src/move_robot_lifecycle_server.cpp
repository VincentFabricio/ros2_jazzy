...
class MoveRobotLifecycleServerNode : public rclcpp_lifecycle::LifecycleNode 
{
public:
    MoveRobotLifecycleServerNode() : LifecycleNode("move_robot_lifecycle_server_cpp") 
    {
        robot_position_ = 50;
    ...
    LifecycleCallbackReturn on_configure(const rclcpp_lifecycle::State &previous_state)
    {
        ...
        move_robot_server_ = rclcpp_action::create_server<MoveRobot>(
            ...
            std::bind(&MoveRobotLifecycleServerNode::goal_callback, this, _1, _2),
            std::bind(&MoveRobotLifecycleServerNode::cancel_callback, this, _1),
            std::bind(&MoveRobotLifecycleServerNode::handle_accepted_callback, this, _1),
...
private:
    ...
    {
        (void)uuid;
        RCLCPP_INFO(this->get_logger(), "Rceived a new goal");
        ...
        {
            RCLCPP_WARN(this->get_logger(), "Invalid position/velocity, reject goal");
            return rclcpp_action::GoalResponse::REJECT;
        }
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<MoveRobotLifecycleServerNode>();

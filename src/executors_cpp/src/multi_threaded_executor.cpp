...
class MultiThreadedExecutorNode : public rclcpp::Node
{
public:
    MultiThreadedExecutorNode() : Node("multi_threaded_executor_cpp")
    {
        ...
        this->timer1_ = this->create_wall_timer(
            1000ms,
            std::bind(&MultiThreadedExecutorNode::callBackTimer1, this),
            cb_group1_);
        this->timer2_ = this->create_wall_timer(
            1000ms,
            std::bind(&MultiThreadedExecutorNode::callBackTimer2, this),
            cb_group2_);
        this->timer3_ = this->create_wall_timer(
            1000ms,
            std::bind(&MultiThreadedExecutorNode::callBackTimer3, this),
            cb_group2_);
    }
...
int main(int argc, char **argv)
{
    ...
    auto node1 = std::make_shared<MultiThreadedExecutorNode>();

...
{
public:
    TurtleBotController(const rclcpp::NodeOptions &options) : LifecycleNode("turtlebot3_controller_cpp", options)
    {
        ...
        const std::shared_ptr<MoveTurtleGoalHandle> goal_handle)
    {
        ...
        while (rclcpp::ok())
        {
            if (now() - start > duration)
            {
                ...
                msg.twist.linear.y  = linear_vel_y;
...
RCLCPP_COMPONENTS_REGISTER_NODE(turtlebot3_controller::TurtleBotController)
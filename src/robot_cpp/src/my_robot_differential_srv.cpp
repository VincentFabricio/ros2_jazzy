...
private:
    ...
    void timer_callback_diff()
    {
        ...
        else if ((start_var == 1) && (action_move == 6))
        {
            msg.linear.x = 0.0;
            msg.angular.z = -PI/4;
            publisher1 -> publish(msg);
            ...
            if (move_counter >= 5)
            {
                ...
                msg.angular.z = 0.0;
                publisher1 -> publish(msg);
                call_actions("diffoff6", 0.0, 0.0, 0.0, 0.0);
            }
            // RCLCPP_INFO(this->get_logger(), 
            //     "Linear: x=%.2f, y=%.2f, z=%.2f | Angular: x=%.2f, y=%.2f, z=%.2f",    
            //     msg.linear.x, msg.linear.y, msg.linear.z,
        ...
        else if ((start_var == 1) && (action_move == 7))
        {
            msg.linear.x = 0.0;
            publisher1 -> publish(msg);
            move_counter ++;
        ...
        std::shared_ptr<my_robot_interfaces::srv::MyRobotSrv::Response> response)
    {
        ...
        else if (request->actions == "diffon6")
        {
            action_move = 6;
        }
        else if (request->actions == "diffon7")
        {
            action_move = 7;
        }

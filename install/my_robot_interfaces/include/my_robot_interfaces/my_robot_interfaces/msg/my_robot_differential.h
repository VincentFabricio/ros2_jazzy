// generated from rosidl_generator_c/resource/idl.h.em
// with input from my_robot_interfaces:msg/MyRobotDifferential.idl
// generated code does not contain a copyright notice

#ifndef MY_ROBOT_INTERFACES__MSG__MY_ROBOT_DIFFERENTIAL_H_
#define MY_ROBOT_INTERFACES__MSG__MY_ROBOT_DIFFERENTIAL_H_

#include "my_robot_interfaces/msg/detail/my_robot_differential__struct.h"
#include "my_robot_interfaces/msg/detail/my_robot_differential__functions.h"
#include "my_robot_interfaces/msg/detail/my_robot_differential__type_support.h"

#endif  // MY_ROBOT_INTERFACES__MSG__MY_ROBOT_DIFFERENTIAL_H_

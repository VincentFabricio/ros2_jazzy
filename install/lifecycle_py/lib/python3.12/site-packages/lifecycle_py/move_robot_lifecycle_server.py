...
class MoveRobotServerNode(LifecycleNode): 
    def __init__(self):
        super().__init__("move_robot_lifecycle_server_py") 
    ...
    def on_configure(self, state: LifecycleNode):
    ...
    def on_cleanup(self, state: LifecycleNode):
        self.undeclare_parameter("robot_name")
        self.robot_name_ = ""
        self.move_robot_server_.destroy()
        return TransitionCallbackReturn.SUCCESS
    
    def on_activate(self, state: LifecycleNode):
    ...
    def on_shutdown(self, state: LifecycleNode):
        self.undeclare_parameter("robot_name")
        self.robot_name_ = ""
        self.move_robot_server_.destroy()
        return TransitionCallbackReturn.SUCCESS
    ...
    def goal_callback(self, goal_request: MoveRobot.Goal):
        ...
        # New goal is valid, abort previous goald and accept new goal
        with self.goal_lock_:
            if self.goal_handle_ is not None and self.goal_handle_.is_active:
                self.goal_handle_.abort()
    ...
    def execute_callback(self, goal_handle: ServerGoalHandle):
        ...
        self.get_logger().info('Exceute goal')
        while  rclpy.ok():
